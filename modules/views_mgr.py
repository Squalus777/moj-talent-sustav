...
def render_manager_view():
    ...
    if menu == "📊 Dashboard":
        ...
        if team_ids:
            ...
            query = f"SELECT * FROM evaluations WHERE period=? AND is_self_eval=0 AND kadrovski_broj IN ({placeholders})"
            params = [current_period] + team_ids
            my_evals = pd.read_sql_query(query, conn, params=params)
        else:
            my_evals = pd.DataFrame()
        ...
        avg_p = my_evals['avg_performance'].mean() if not my_evals.empty else 0.0
        c3.metric("Prosjek Tima", f"{avg_p:.2f}")
        ...
        with t2:
            if not my_team.empty:
                ...
                if sel_emp:
                    ...
                    hist = pd.read_sql_query("SELECT period, avg_performance, avg_potential, category, is_self_eval FROM evaluations WHERE kadrovski_broj=? ORDER BY period", conn, params=(kid,))
                    if not hist.empty:
                        hist['Izvor'] = hist['is_self_eval'].apply(lambda x: 'Samoprocjena' if x==1 else 'Voditelj')
                        fig_h = px.line(hist, x="period", y="avg_performance", color="Izvor", markers=True)
        ...
        with t3:
            ...
            st.divider()
            query_del = """
                SELECT e_target.ime_prezime as 'Zaposlenik', e_delegate.ime_prezime as 'Mentor', d.status as 'Status'
                ...
                if s:
                    ...
                    with st.form("ng"):
                        ...
                        if st.form_submit_button("Kreiraj"):
                            ...
                            conn.commit(); st.success("Kreirano!"); st.rerun()
        st.markdown("---")
            ...
            with st.expander(f"👤 {r['ime_prezime']} (Ciljevi: {len(goals)})"):
                for _, g in goals.iterrows():
                    ...
                    if st.button("💾 Spremi KPI", key=f"s_{gid}"):
                        ...
                        conn.commit(); st.toast("Spremljeno!"); st.rerun()
    ...
    # 3. UNOS PROCJENA
        ...
        for _, emp in my_team.iterrows():
            ...
            # Naslov expandera
            status_txt = "Završeno" if is_locked else "U tijeku" if r is not None else "Nije započeto"
            
            with st.expander(f"{'🔒' if is_locked else '✏️'} {emp['ime_prezime']} ({status_txt})"):
                
                # --- GAP ANALIZA ---
                if not s_eval.empty:
                    st.info("💡 Zaposlenik je ispunio samoprocjenu. Ovdje je usporedba:")
                    se = s_eval.iloc[0]
                    # FIX: Prisilna konverzija u int za izbjegavanje grešaka
                    m_vals = [int(r[f'p{i}']) for i in range(1,6)] + [int(r[f'pot{i}']) for i in range(1,6)] if r is not None else [0]*10
                    s_vals = [int(se[f'p{i}']) for i in range(1,6)] + [int(se[f'pot{i}']) for i in range(1,6)]
                    
                    labels = [x['title'] for x in METRICS['p']] + [x['title'] for x in METRICS['pot']]
                    
                    gap_df = pd.DataFrame({
                        "Metrika": labels,
                        "Zaposlenik": s_vals,
                        "Manager": m_vals,
                        "Razlika": [m - s for m, s in zip(m_vals, s_vals)]
                    })
                    st.dataframe(gap_df.style.applymap(lambda x: 'color: red' if x < -1 else ('color: green' if x > 1 else 'color: black'), subset=['Razlika']), use_container_width=True)
                ...
                if is_locked and r is not None:
                    ...
                    with c1:
                        st.write("**Učinak:**")
                        for i, m in enumerate(METRICS["p"]): st.write(f"{m['title']}: **{r[f'p{i+1}']}**")
                    with c2:
                        st.write("**Potencijal:**")
                        for i, m in enumerate(METRICS["pot"]): st.write(f"{m['title']}: **{r[f'pot{i+1}']}**")
                    if st.button("🖨️ PDF", key=f"pdf_{kid}"):
                        ...
                        <h3>Učinak</h3><ul>{"".join([f"<li>{METRICS['p'][i]['title']}: {r[f'p{i+1}']}</li>" for i in range(5)])}</ul>
                        <h3>Potencijal</h3><ul>{"".join([f"<li>{METRICS['pot'][i]['title']}: {r[f'pot{i+1}']}</li>" for i in range(5)])}</ul>
                        <p><b>Akcijski plan:</b> {r['action_plan']}</p><script>window.print();</script></body></html>"""
                        components.html(html_pdf, height=400)
                else:
                    with st.form(f"eval_form_{kid}"):
                        ...
                        with c1: 
                            st.subheader("Učinak")
                            # FIX: int() konverzija
                            p = [render_metric_input(METRICS["p"][i], f"p{i}_{kid}", int(r[f'p{i+1}']) if r is not None else 3) for i in range(5)]
                        with c2: 
                            st.subheader("Potencijal")
                            # FIX: int() konverzija
                            pot = [render_metric_input(METRICS["pot"][i], f"pot{i}_{kid}", int(r[f'pot{i+1}']) if r is not None else 3, "pot") for i in range(5)]
                        
                        plan = st.text_area("Bilješke / Akcijski plan:", r['action_plan'] if r is not None else "")
                        ...
                        if st.form_submit_button("💾 Spremi Nacrt"):
                            ...
                            conn.execute("""INSERT INTO evaluations 
                                 ...
                                 *p, *pot, ap, at, cat, plan, 'Draft', datetime.now().strftime("%Y-%m-%d"), company_id, 0))
                            conn.commit(); st.success("Nacrt spremljen!"); st.rerun()
                    
                    if r is not None:
                        st.info("Kada završite, zaključajte procjenu.")
                        lock = st.checkbox(f"Potvrđujem konačnost za {emp['ime_prezime']}", key=f"lock_{kid}")
    ...
    # 4. IDP (PUNI PRIKAZ S OPISIMA)
        ...
        for _, emp in team.iterrows():
            ...
            with st.expander(f"📄 {emp['ime_prezime']}"):
                ...
                st.markdown('Primjer: "Preuzeti potpunu organizaciju jednog internog događaja od plana do realizacije."')
                ...
                st.markdown('Primjer: "Kratki 1-na-1 sastanak s Voditeljem projekata jednom mjesečno radi feedbacka na vođenje sastanaka."')
                ...
                st.markdown('Primjer: "Završiti Excel Advanced tečaj na Udemy platformi."')
