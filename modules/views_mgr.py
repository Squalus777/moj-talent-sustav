...
def render_manager_view():
    ...
    if menu == "📊 Dashboard":
        ...
        # POPRAVLJENO: Dohvaćam procjene za MOJ TIM (tko god da ih je unio, uključujući delegate)
        # Isključujemo samoprocjene (is_self_eval=0)
        ...
        with t1:
            if not my_evals.empty:
                # POPRAVLJENO: 9 polja (linije na 2.5 i 4.0)
        ...
        with t2:
            if not my_team.empty:
                ...
                if sel_emp:
                    ...
                    # Prikazujemo i manager procjene i samoprocjene
    ...
    # 3. UNOS PROCJENA (GAP ANALIZA + LOCK GUMB)
        ...
        for _, emp in my_team.iterrows():
            ...
            # Tražimo procjenu (bilo moju ili od delegata)
            ...
            with st.expander(f"{'🔒' if is_locked else '✏️'} {emp['ime_prezime']}"):
                ...
                if not s_eval.empty:
                    ...
                    se = s_eval.iloc[0]
                    # Ako manager još nema procjenu, stavi nule
                    m_vals = [r[f'p{i}'] for i in range(1,6)] + [r[f'pot{i}'] for i in range(1,6)] if r is not None else [0]*10
                    s_vals = [se[f'p{i}'] for i in range(1,6)] + [se[f'pot{i}'] for i in range(1,6)]
                    
                    gap_df = pd.DataFrame({
                        "Metrika": [x['title'] for x in METRICS['p']] + [x['title'] for x in METRICS['pot']],
                        "Zaposlenik": s_vals,
                        "Manager": m_vals,
                ...
                if is_locked and r is not None:
                    ...
                    if st.button("🖨️ PDF", key=f"pdf_{kid}"):
                        html_pdf = f"""<html><body onload='window.print()'><h1>Procjena: {emp['ime_prezime']}</h1><p>Kategorija: {r['category']}</p></body></html>"""
                        components.html(html_pdf, height=400)
                else:
                    with st.form(f"eval_form_{kid}"):
                        c1, c2 = st.columns(2)
                        with c1: p = [render_metric_input(METRICS["p"][i], f"p{i}_{kid}", r[f'p{i+1}'] if r is not None else 3) for i in range(5)]
                        with c2: pot = [render_metric_input(METRICS["pot"][i], f"pot{i}_{kid}", r[f'pot{i+1}'] if r is not None else 3, "pot") for i in range(5)]
                        plan = st.text_area("Bilješke:", r['action_plan'] if r is not None else "")
                        
                        # GUMB SPREMI (KAO NACRT)
    ...
    # 4. IDP (PUNI PRIKAZ S OPISIMA)
        ...
        for _, emp in team.iterrows():
            ...
            with st.expander(f"📄 {emp['ime_prezime']}"):
                ...
                st.markdown('Primjer: "Preuzeti potpunu organizaciju..."')
                ...
                st.markdown('Primjer: "Sastanak s mentorom..."')
                ...
                st.markdown('Primjer: "Excel tečaj..."')
                ...
                sup = st.multiselect("Potrebno:", ["Budžet", "Vrijeme", "Mentorstvo", "Alati"], default=[x.strip() for x in d.get('support_needed','').split(',') if x.strip()], key=f"sup_{eid}")
