...
def render_employee_view():
    ...
    del_tasks = pd.read_sql_query("""SELECT d.*, e.ime_prezime as target_name, e.radno_mjesto as target_role, e.department as target_dept FROM delegated_tasks d JOIN employees_master e ON d.target_id = e.kadrovski_broj WHERE d.delegate_id = ? AND d.period = ? AND d.status = 'Pending'""", conn, params=(username, current_period))
    ...
    # 2. CILJEVI
    with tabs[1]:
        goals = pd.read_sql_query("SELECT title, progress FROM goals WHERE kadrovski_broj=? AND period=?", conn, params=(username, current_period))
        for _, g in goals.iterrows(): st.write(f"**{g['title']}**"); st.progress(g['progress']/100)
    ...
    with tabs[2]:
        ...
        with st.form("self_eval"):
            ...
            if st.form_submit_button("Spremi samoprocjenu"):
                ...
                nm, rm, dp = (emp_res[0], emp_res[1], emp_res[2]) if emp_res else (username, "Employee", "N/A")
    ...
    with tabs[3]:
        ...
        with c2:
            st.subheader("📊 Pulse"); score = st.slider("Zadovoljstvo (1-10):", 1, 10, 7)
            if st.button("Pošalji"): conn.execute("INSERT INTO pulse_responses (score, timestamp, company_id) VALUES (?,?,?)", (score, datetime.now().strftime("%Y-%m-%d"), company_id)); conn.commit(); st.success("Hvala!")

    # 5. 1-ON-1
    with tabs[4]:
        st.subheader("📅 Sastanci")
        notes = pd.read_sql_query("SELECT manager_id, date, notes FROM meeting_notes WHERE kadrovski_broj=? ORDER BY date DESC", conn, params=(username,))
        if not notes.empty: [st.write(f"**Sastanak s {n['manager_id']}** ({n['date']})\n> {n['notes']}\n---") for _, n in notes.iterrows()]
                ...
                with st.expander(f"Zadatak: Procijeni {task['target_name']}"):
                    with st.form(f"task_eval_{task['id']}"):
                        ...
                        if st.form_submit_button("Pošalji"):
                            ...
                            cat = calculate_category(ap, apot)
                            # FIX: Upisujemo točne podatke ciljanog zaposlenika
                            conn.execute("""INSERT INTO evaluations 
                                (period, kadrovski_broj, ime_prezime, radno_mjesto, department, manager_id, 
                                p1, p2, p3, p4, p5, pot1, pot2, pot3, pot4, pot5, 
                                avg_performance, avg_potential, category, action_plan, status, feedback_date, company_id, is_self_eval) 
                                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                                (current_period, task['target_id'], task['target_name'], task['target_role'], task['target_dept'], username, 
                                 *tp, *tpot, ap, apot, cat, "Delegirana procjena", 'Submitted', datetime.now().strftime("%Y-%m-%d"), company_id, 0))
